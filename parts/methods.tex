...
The impact of this new rule for components cooperation is highlithed in the following.
...
In this work, we aim to model a biological  system according to his dynamic. For that purpose, we choose to build a PH model from the biological system reprensented as  an RSTC network.
...
as biochemical reactions in the RSTC network, and propose their PH representation. 

%les ajouts pour les patterns 
We build an algorithm that automatically browse the graph node by node and detect all patterns in the graph. More precisely, for each node (output node of the pattern) we  call a recursive procedure,
that  allow  to detect a minimal set of node(input node of the pattern) that has a direct influence on that node. This set of nodes plus the output node and the way  are linked are a pattern for us. 
...
and its type to another procedure which  translates the pattern into the process hitting formalism. This transformation  takes care of different cases (cooperation, synchronization, simple activation, simple inhibition,...)
...
and the stochasticity absorption $sa$. These two parameters are estimated according to the expression profile of time-series data of the experiment
...
level $2$. In  this way we can automatically determine the differents levels of expression of the TSD. 
...
\begin{itemize}
 \item \textbf{Receptor layer: E-cadherin.} We choose the  pulse signal for the input node E-cadherin, that is active for a duration of $x$ units times in average. We made this 
 choice to take into account the average time of the calcium stimuli effect.
 \item \textbf{Signaling layer: signaling proteins.} At this layer, the components are activated and inhibited according to the same rate and the same stochasticity absorption factor.
 The values of these parameters were selected by considering the delay of signal transduction from the entry node (E-cadherin) to the output node (genes).[imposing reachability
...
The initial conditions of the simulation are summarised in \pref{fig:initialCondition} where we can clearly see the initial values chosen at each layer for our model.
