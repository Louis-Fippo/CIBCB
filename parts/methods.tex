...
\SWITCH {$nocp$}
 \CASE {2}
  \SWITCH {$type$}
 \CASE {A}
  \STATE out.write ($tabPat[1]$ 1 $->$ $tabPat[0]$ $0$ $1$ $r_{a}$ $sa_{a}$);  \COMMENT{Component $tabPat[1]$ activates component $tabPat[0]$ with $r = r_{a}$ and $sa = sa_{a}$ }     
\ENDCASE
\CASELINE {I}
  \STATE out.write ($tabPat[1]$ 0 $->$ $tabPat[0]$ $1$ $0$ $r_{i}$ $sa_{i}$);  \COMMENT{Component $tabPat[1]$ inhibits component $tabPat[0]$ with $r = r_{i}$ and $sa = sa_{i}$ }  
%\DEFAULT
...
\CASE {3}
  \SWITCH {$type$}
 \CASE {C}
   \STATE out.write (coop ([$tabPat[2]$;$tabPat[1]$)] $->$ $tabPat[0]$ $0$ $1$, [[$1$;$1$]] ); \COMMENT{Cooperation between $tabPat[1]$ and $tabPat[2]$ to activate $tabPat[0]$}
        
\ENDCASE
\CASELINE {S}
  \STATE out.write (coop ([$tabPat[2]$;$tabPat[1]$)] $->$ $tabPat[0]$ $0$ $1$, [[$1$;$1$],[$1$;$0$],[$0$;$1$]]); \COMMENT{Synchronization between $tabPat[1]$ and $tabPat[2]$ to activate $tabPat[0]$}
\DEFAULT
 \STATE out.write ((*unknow pattern*));
 \ENDDEFAULT
\ENDSWITCH
...
In the RSTC network, we find $11$(to be precise...) regulatory patterns (see Appendix \ref{table_patterns}).     %TODO
...
in \pref{fig:estimationParameter}. Each  $t_{i}$ represent the time in which we assume that a component move from one process to another. Therefore the action that leads this 
